...
public class MyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("list")) {
			...
			// 4.
			if (res>0) {
				//request.setAttribute("list", biz.selectList()); // db에서 값 가져와서
				//dispatch(request, response, "mylist.jsp");
				response.sendRedirect("myservlet.do?command=list");
				
			} else {
					//response.sendRedirect("myinsert.jsp");
					response.sendRedirect("myservlet.do?commnad=insertform");
			}	
			...
			if (res>0) {
				...
				response.sendRedirect("myservlet.do?command=selectone&seq="+seq);
					...
					//response.sendRedirect("myinsert.jsp");
					response.sendRedirect("myservlet.do?commnad=updateform&seq="+seq);
			}	
			...
			if (res>0) {
				...
				response.sendRedirect("myservlet.do?command=list");
					...
					//response.sendRedirect("myinsert.jsp");
					response.sendRedirect("myservlet.do?commnad=selectone&seq="+seq);
			}	
		}
